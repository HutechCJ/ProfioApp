...
public sealed class CheckAuthorizationQueryHandler : IRequestHandler<CheckAuthorizationQuery, AccountDto>
{
  ...
  public CheckAuthorizationQueryHandler(UserManager<ApplicationUser> userManager, IMapper mapper,
    ITokenService tokenService, IUserAccessor userAccessor)
  ...
  public async Task<AccountDto> Handle(CheckAuthorizationQuery request, CancellationToken cancellationToken)
  {
    ...
    var user = await _userManager.Users
                 .Include(x => x.Staff)
                 .SingleOrDefaultAsync(x => x.UserName != null && x.UserName.Equals(_userAccessor.UserName),
                   cancellationToken)
